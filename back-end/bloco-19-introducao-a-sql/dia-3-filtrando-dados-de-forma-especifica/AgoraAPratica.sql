...
-- Escreva uma query para exibir todas as informações dos fornecedores que são empresas limitadas 
-- (LTDA). Ordene os resultados em ordem alfabética decrescente.
SELECT * FROM Fornecedores
WHERE name LIKE '%LTDA'
ORDER BY name DESC;
-- Escreva uma query para exibir o número de empresas (fornecedores) que contém a letra F no código.
SELECT COUNT(*) as 'número de empresas (fornecedores) que contém a letra F no código' FROM Fornecedores
WHERE code LIKE '%F%';
...
SELECT COUNT(*) AS 'número de vendas feitas' FROM Vendas
